...
// Remove ProgressBar import if it's not being used anymore
// import ProgressBar from '@badrap/bar-of-progress'; // No longer needed
import { CheckIcon } from '@heroicons/react/solid'; // Import the check icon
                        ...
                        <ul className="space-y-4">
                            {planSteps.map((step, index) => (
                                <li key={index} className="flex items-start">
                                    ...
                                    />
                                    <span className="text-lg flex-1">{step}</span>
                                    {progress[index] && <CheckIcon className="h-5 w-5 text-green-500 ml-2" />}
                                </li>
                            ))}
                        </ul>
