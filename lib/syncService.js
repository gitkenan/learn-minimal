...
export const syncService = {
  ...
  async toggleTaskCompletion(planId, sectionId, itemId) {
    ...
    try {
      ...
      // Check for existing calendar task
      const { data: calendarTask, error: taskError } = await supabase
        .from('calendar_tasks')
        .select('id')
        .eq('plan_id', planId)
      ...
      if (taskError && taskError.code !== 'PGRST116') { // Ignore "No rows found" error
        throw taskError;
      }

      // Only sync if calendar task exists
      ...
      if (calendarTask?.id) {
        transactionPayload.p_calendar_task_id = calendarTask.id;
      }
